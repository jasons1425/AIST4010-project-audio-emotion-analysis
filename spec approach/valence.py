...
train_loader = get_loader(train_data, train_labels,
                          batch_size=BATCH, transform=spectrum_transform_crop(crop_size=217))
valid_loader = get_loader(valid_data, valid_labels,
                          batch_size=BATCH)
...
# DROPOUT = 0.5
# device = "cuda" if torch.cuda.is_available() else "cpu"
# model = PlainCNN(229376, 1, fcs=FC, dropout=DROPOUT).half().to(device)
