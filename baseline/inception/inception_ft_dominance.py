...
train_labels = get_labels(train_ids)[:, 2].reshape(-1, 1) / 9  # map the 9-point scale to 0-1 scale
valid_labels = get_labels(valid_ids)[:, 2].reshape(-1, 1) / 9  # map the 9-point scale to 0-1 scale
# if resize is None, the image dimension will be 217 * 334 (H * W)
train_transform = spectrum_transform(resize=299, norm=True)
...
model = InceptionSpecModel(inception_v3, 1, fcs=FC, dropout=DROPOUT).half().to(device)
...
with open("inception_dominance_loss.npy", 'wb') as f:
    np.save(f, np.array(losses['valid']))
torch.save(model.state_dict(), f"inception_dominance.pth")
