...
class WaveNet(nn.Module):
    def __init__(self, out_dim, fe_dim=2048,
        ...
        if freeze:
            layers_freezed = [
                self.wavecnn.pre_conv0,
                self.wavecnn.pre_block1,
                self.wavecnn.pre_block2,
                self.wavecnn.pre_block3,
                self.wavecnn.pre_block4,
                self.wavecnn.conv_block2,
                self.wavecnn.conv_block3,
                self.wavecnn.conv_block4,
                self.wavecnn.conv_block5,
                self.wavecnn.conv_block6,
            ]
            for idx, layer in enumerate(layers_freezed):
                if idx >= freeze:
                    break
    ...
    def forward(self, x):
        spectrum_features = self.wavecnn(x, None)['embedding']
